...
export default {
  login: async function(clientId, game:'bike'|'cube'|'clone'|'miner'|'merge'|'twerk', services: Services) {
    try {
  ...
  registerEvent: async function(clientToken, game:'bike'|'cube'|'clone'|'miner'|'merge'|'twerk', services: Services) {
    try {
      const agent = services.proxy.genProxyAgent()
  ...
  generateKey: async function(clientToken, game:'bike'|'cube'|'clone'|'miner'|'merge'|'twerk', services: Services) {
    try {
