...
export default async function generateKeys (keyCount:number = 1, ctx:Context,chatId, messageId, progress:number, username:string, gameId: Games, edit:boolean = true): Promise<Awaited<string | void>[]> {
  ...
  async function generateKeyProcess() {
    ...
    try {
      clientToken = await generateKeysReducers.login(clientId, game.app_token, proxy, androidUserAgent,undefined, services);
    } catch (error) {
    ...
    for (let i: number = 0; i < (PENDING_AMOUNT_ITERATIONS * 2); i++) {
      try {
        ...
        const hasCode = await generateKeysReducers.registerEvent(clientToken, game.promo_id, proxy, androidUserAgent, undefined, services);
        if (hasCode) {
          break;
        }
      } catch (error) {
    ...
    try {
      const key = await generateKeysReducers.generateKey(clientToken, game.promo_id, proxy, androidUserAgent, undefined, services);
      return key;
    } catch (error) {
